...
export const useNotificationSocket = ({
    ...
    // Effect for Cleanup
    //------------------------------------------------------
    // Effect này chỉ chạy 1 lần khi hook unmount để đảm bảo ngắt kết nối
    useEffect(() => {
        return () => {
            console.log('WebSocket: Cleaning up hook...');
            handleDisconnect(false); // Dọn dẹp khi component/hook bị hủy
        };
    }, [handleDisconnect]); // Chỉ phụ thuộc vào handleDisconnect
